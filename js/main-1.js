...
async function handleSubmit(e) {
  ...
  // JSON method - most reliable with Formspree
  const formData = new FormData(form);
  const data = {};
  formData.forEach((val, key) => { data[key] = val; });

  try {
    const response = await fetch('https://formspree.io/f/mnjwyowd', {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      const msg = (result.errors || []).map(e => e.message).join(', ') || 'Dobara try karein.';
    ...
    // Fallback with FormData
    try {
      const r2 = await fetch('https://formspree.io/f/mnjwyowd', {
        method: 'POST',
      ...
      errorMsg.textContent = 'Submit nahi hua. Call karein: +91 99270 14521';
      errorMsg.style.display = 'block';
      btn.textContent = 'Book Appointment →';
      btn.disabled = false;
      btn.style.opacity = '1';
  ...
  // Load on DOMContentLoaded and also after small delay as fallback
  loadAllLazyImages();
  setTimeout(loadAllLazyImages, 500);
